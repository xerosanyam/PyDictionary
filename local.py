...
"""
													#when meaning of a word is not found
html2_doc="""												
<?xml version="1.0" encoding="utf-8" ?>
<entry_list version="1.0">
	<suggestion>pickax</suggestion>
	<suggestion>pitchout</suggestion>
	<suggestion>pickaxe</suggestion>
	<suggestion>paycheck</suggestion>
	<suggestion>package</suggestion>
	<suggestion>picked</suggestion>
	<suggestion>picket</suggestion>
	<suggestion>pitched</suggestion>
	<suggestion>packed</suggestion>
	<suggestion>packet</suggestion>
	<suggestion>upchuck</suggestion>
	<suggestion>backache</suggestion>
	<suggestion>paddock</suggestion>
	<suggestion>peacock</suggestion>
	<suggestion>poached</suggestion>
	<suggestion>pickled</suggestion>
	<suggestion>packaged</suggestion>
	<suggestion>backchat</suggestion>
	<suggestion>poetical</suggestion>
	<suggestion>optical</suggestion>
</entry_list>
"""
try:
	import requests
...
while ch=='y':
	os.system('clear')    
	keyword = raw_input('\nEnter the word :')		#Takes input
	print("Connecting")
	soup = BeautifulSoup(html_doc,"xml")
	if soup.select("dt")==[] :
		print("Meaning could not be found.")
		quit()
	examples=[]
	#print(len(list(soup.dt.descendants)))
	for link in soup.find_all('vi'):	#Store All Examples
		examples.append(link.text)
		if len(examples)==3:
			break
	#print(len(examples))
	#print(len(list(soup.dt.descendants)))
	for tag in soup.dt.findAll(True):
		tag.decompose()
	#print(len(list(soup.dt.descendants)))
	meaning=soup.dt.get_text()
	label = soup.fl.string
	print("Label\t:"+label)
